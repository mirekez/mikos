...
if [[ ! -f "$build/stamps/build-gcc-linux-stage2" ]] &&
    (( $(df -Pk "$build" | awk 'END {print $4}') < 8 * 1024 * 1024 )); then
  echo "The userspace toolchain build needs at least 8 GiB free at $build." >&2
...
for directory in build-binutils-linux build-gcc-linux-stage1 build-glibc-linux-rv32ima-ilp32; do
